# NOR Manuscript Scripts: Analysis Pipeline
...
### 3.2. Extract Behavior & Kinematics Data from .mat data files
    ...
    2. In MATLAB File Explorer, navigate to `Obj1` directory 
...
This generates `compiled_behavior.xlsx` (containing frame numbers of all object exploration bouts for objects 1 & 2) and `compiled_kinematics.xlsx` (containing extracted total distance traveled, average velocities, and average accelerations for head and midback smoothed keypoint-tracking points. If other or additional data points are to be extracted modify text under     `T_kinematics = table()` in  `extract_behavior_kinematics.m` file.
...
#### 3.3.1. Combine Obj1 and Obj2 Data 
    ...
    2. Open Terminal/Anaconda Prompt and enter the following (replace '/Path/' sections with correct full Paths of specified files):
...
#### 4.2.a. Crop `Explore_Obj2` Labels 
    ...
    2. Open Terminal/Anaconda Prompt, change to the directory referenced in previous step:
         cd /Path/to/Explore_Obj2_videos
    3. Activate NORenv then enter following:
         conda activate NORenv
...
This will create a new folder within present working directory that contains the cropped labels, use videos from this directory when `Explore_Obj2`-labeled videos are required.
